...
function reducer() {
  ...
  switch (action.type) {
    ...
    case loadData_types/* FETCH_CATEGORIES */.Hw:
      {
        ...
        var categories = Object.values(_data.reduce(function (a, _ref) {
    ...
    case loadData_types/* FETCH_SPECIFIC_CATEGORIES */.q:
      {
        ...
        var catNewItems = sortArrayDesc(_data2, "created");

        var cat1 = _data2.filter(function (product) {
          return product.categories[0] && product.categories[0].slug === "giochi";
        });

        var cat2 = _data2.filter(function (product) {
    ...
    case loadData_types/* FETCH_HIGHEST_VALUE */.MD:
      {
        ...
        var topValue = Math.max.apply(Math, _data3.map(function (element) {
    ...
    case loadData_types/* GET_ITEM */.rt:
      {
        var _data4 = state.data;
        var key = Number(action.payload.key);

        var selectedItem = _data4.find(function (item) {
          return item.id === key;
        }); // FIXARE ? 🧨
...
//# sourceMappingURL=index.4350a5a3d058d197659b.js.map